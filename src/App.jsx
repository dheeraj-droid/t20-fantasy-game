...
// --- CONSTANTS ---
const INITIAL_SYSTEM_TIME = new Date("2026-04-30T19:30:00");
...
const Squads = {
  ...
  "KKR": [
    ...
    { name: "Vaibhav Arora", role: "BOWL", isOverseas: false }, { name: "Matheesha Pathirana", role: "BOWL", isOverseas: true }, { name: "Kartik Tyagi", role: "BOWL", isOverseas: false }, { name: "Prashant Solanki", role: "BOWL", isOverseas: false }, { name: "Saurabh Dubey", role: "BOWL", isOverseas: false }, { name: "Harshit Rana", role: "BOWL", isOverseas: false }, { name: "Umran Malik", role: "BOWL", isOverseas: false }, { name: "Sunil Narine", role: "BOWL", isOverseas: true }, { name: "Varun Chakravarthy", role: "BOWL", isOverseas: false },{ name: "Blessing Muzarabani", role: "BOWL", isOverseas: false },
  ...
  "SRH": [
    ...
    { name: "Abhishek Sharma", role: "AR", isOverseas: false }, { name: "Nitish Kumar Reddy", role: "AR", isOverseas: false }, { name: "Harshal Patel", role: "AR", isOverseas: false }, { name: "Liam Livingstone", role: "AR", isOverseas: true }, { name: "Kamindu Mendis", role: "AR", isOverseas: true }, { name: "Brydon Carse", role: "AR", isOverseas: true }, { name: "David Payne", role: "AR", isOverseas: true }, { name: "Harsh Dubey", role: "AR", isOverseas: false }, { name: "Shivang Kumar", role: "AR", isOverseas: false }, { name: "Krains Fuletra", role: "AR", isOverseas: false }
  ]
};
...
const getFranchise = (playerName) => {
  for (const team in Squads) {
    if (Squads[team].some(p => p.name === playerName)) return team;
  }
  return null;
};
...
const calculateRoundScore = (roundMatchIds, lineup, activeChip, chipNomination, team, matchResults, matchDetails, matchSubmissionTimes, playerRegistry = {}) => {
  ...
  if (activeChip === 'loyalty') {
    Object.keys(playerStats).forEach(pName => {
      const teamId = getFranchise(pName);
      if (teamId) {
        franchiseCounts[teamId] = (franchiseCounts[teamId] || 0) + 1;
      }
    });
  }
  ...
  Object.keys(playerStats).forEach(pName => {
    ...
    if (activeChip === 'loyalty') {
      const pFranchise = getFranchise(pName);
      if (pFranchise && franchiseCounts[pFranchise] >= 4) {
        chipMult = 2;
      }
    }
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {team.activeChip && (
                              <div className="flex items-center gap-1 bg-indigo-500/20 px-2 py-1 rounded-lg border border-indigo-500/30">
                                ...
                                {team.activeChip === 'loyalty' && <ShieldCheck size={12} className="text-emerald-400" />}
                                <span className="text-[9px] font-black uppercase text-indigo-200 tracking-wider">
                                  {team.activeChip === 'pom' ? `POTM (${team.chipNomination})` : team.activeChip === 'supersub' ? `SUB (${team.chipNomination})` : team.activeChip === 'loyalty' ? 'Loyalty Bonus' : team.activeChip}
                                </span>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="w-full md:w-2/3 overflow-y-auto p-8 scrollbar-thin scrollbar-thumb-white/10">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {(() => {
                        ...
                        const squadFranchiseCounts = {};
                        if (editingTeam.activeChip === 'loyalty') {
                          editingTeam.playingXINames.forEach(n => {
                            const f = getFranchise(n);
                            if (f) squadFranchiseCounts[f] = (squadFranchiseCounts[f] || 0) + 1;
                          });
                        }
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="flex items-center gap-2">
                                    ...
                                    {editingTeam.activeChip === 'loyalty' && isInXI && squadFranchiseCounts[playerTeam] >= 4 && (
                                      <span className="bg-emerald-500 text-white text-[8px] font-black px-1.5 rounded flex items-center gap-1">
                                        <ShieldCheck size={8} /> LOYALTY (2x)
                                      </span>
                                    )}
                                  </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-6">
                        {[
                          ...
                          { id: 'loyalty', label: 'Loyalty Bonus', icon: <ShieldCheck size={14} /> },
                      ...
                      {(editingTeam.activeChip === 'pom' || editingTeam.activeChip === 'supersub') && (
                        <div className="mb-6 animate-in slide-in-from-top-2 fade-in duration-300">
                          <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                            {editingTeam.activeChip === 'supersub' ? 'Nominate Bench Player' : 'Nominate Player'}
                          </h4>
                          <select
